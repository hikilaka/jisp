...
public final class Tokenizer {
    ...
    public Optional<Token> next() {
        ...
        case '(':
        case '[':
            return Optional.of(new Token(TokenType.OPEN_PAREN,
                    buffer.take(1).get(), row++, column));

        case ')':
        case ']':
            return Optional.of(new Token(TokenType.CLOSED_PAREN,
                    buffer.take(1).get(), row++, column));
    ...
    private Optional<Token> parseString() {
        ...
        if (offset > 0) {
            ...
            buffer.advance(1);
            return Optional.of(new Token(TokenType.STRING, value, row - offset, column));
        } else {
            return Optional.empty();
        }
    }
    ...
    private Optional<Token> parseInteger() {
        ...
        if (offset > 0) {
            row += offset;
            return Optional.of(new Token(TokenType.NUMBER,
                    buffer.take(offset).get(), row - offset, column));
        } else {
    ...
    private Optional<Token> parseSymbol() {
        ...
        if (offset > 0) {
            row += offset;
            return Optional.of(new Token(TokenType.SYMBOL,
                    buffer.take(offset).get(), row - offset, column));
        } else {
            return Optional.empty();
        }
